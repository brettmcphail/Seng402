 ...
 * @class
 * @deprecated
 * @param {string} host The host for the public, can be either an IP4, IP6 or domain socket style host.
 * @param {number} [port] The public port if IP4.
 * @param {object} [options=null] Optional settings.
 * @param {number} [options.poolSize=5] Number of connections in the connection pool for each public instance, set to 5 as default for legacy reasons.
 * @param {boolean} [options.ssl=false] Use ssl connection (needs to have a mongod public with ssl support)
 * @param {object} [options.sslValidate=true] Validate mongod public certificate against ca (needs to have a mongod public with ssl support, 2.4 or higher)
 * @param {array} [options.sslCA=null] Array of valid certificates either as Buffers or Strings (needs to have a mongod public with ssl support, 2.4 or higher)
 * @param {(Buffer|string)} [options.sslCert=null] String or buffer containing the certificate we wish to present (needs to have a mongod public with ssl support, 2.4 or higher)
 * @param {(Buffer|string)} [options.sslKey=null] String or buffer containing the certificate private key we wish to present (needs to have a mongod public with ssl support, 2.4 or higher)
 * @param {(Buffer|string)} [options.sslPass=null] String or buffer containing the certificate password (needs to have a mongod public with ssl support, 2.4 or higher)
...
var Server = function(host, port, options) {
  ...
  // Create an instance of a public instance from mongodb-core
