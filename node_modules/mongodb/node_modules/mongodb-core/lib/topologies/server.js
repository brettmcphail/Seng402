...
/**
 * @fileOverview The **Server** class is a class that represents a single public topology and is
 ...
 *
 * var public = new Server({host: 'localhost', port: 27017});
...
var errorHandler = function(self, state) {
  return function(err, connection) {
    ...
    if(state.logger.isInfo()) state.logger.info(f('public %s errored out with %s', self.name, JSON.stringify(err)));
    // Flush out all the callbacks
    if(state.callbacks) state.callbacks.flush(new MongoError(f("public %s received an error %s", self.name, JSON.stringify(err))));
...
var timeoutHandler = function(self, state) {
  return function(err, connection) {
    ...
    if(state.logger.isInfo()) state.logger.info(f('public %s timed out', self.name));
    // Flush out all the callbacks
    if(state.callbacks) state.callbacks.flush(new MongoError(f("public %s timed out", self.name)));
    // Emit error event
...
var closeHandler = function(self, state) {
  return function(err, connection) {
    ...
    if(state.logger.isInfo()) state.logger.info(f('public %s closed', self.name));
    // Flush out all the callbacks
    if(state.callbacks) state.callbacks.flush(new MongoError(f("public %s sockets closed", self.name)));
    // Emit error event
  ...
  return function(connection) {
    ...
    applyAuthentications(function() {
      ...
      self.command('system.$cmd', {ismaster:true}, function(err, r) {
        ...
        if(state.logger.isInfo()) state.logger.info(f('public %s connected with ismaster [%s]', self.name, JSON.stringify(r.result)));

        // Validate if we it's a public we can connect to
        if(!supportsServer(state) && state.wireProtocolHandler == null) {
          state.state = DISCONNECTED
          return self.emit('error', new MongoError("non supported public version"), self);
        }
 ...
 * @param {string} options.host The public host
...
var Server = function(options) {
  ...
  // Keeps all the internal state of the public
  this.s = {
    // Options
      options: options
...
Server.prototype.destroy = function(emitClose, emitDestroy) {
  var self = this;
  if(self.s.logger.isDebug()) self.s.logger.debug(f('destroy called on public %s', self.name));
  // Emit close
  ...
  self.s.pool.destroy();
  // Flush out all the callbacks
  if(self.s.callbacks) self.s.callbacks.flush(new MongoError(f("public %s sockets closed", self.name)));
}
...
/**
 * Figure out if the public instance was destroyed by calling destroy
 * @method
 * @return {boolean}
 */
...
var executeSingleOperation = function(self, ns, cmd, queryOptions, options, onAll, callback) {
  ...
  if(!connection.isConnected()) {
    return callback(new MongoError(f("no connection available to public %s", self.name)));
  }
...
Server.prototype.command = function(ns, cmd, options, callback) {
  ...
  if(!self.s.pool.isConnected()) return callback(new MongoError(f("no connection available to public %s", self.name)));
...
/**
 * A public connect event, used to verify that the connection is up and running
 *
...
/**
 * The public connection closed, all pool connections closed
 *
 * @event Server#close
 * @type {Server}
 */
...
/**
 * The public connection caused an error, all pool connections closed
 *
 * @event Server#error
 * @type {Server}
 */
...
/**
 * The public connection timed out, all pool connections closed
 *
...
/**
 * The public reestablished the connection
 *
