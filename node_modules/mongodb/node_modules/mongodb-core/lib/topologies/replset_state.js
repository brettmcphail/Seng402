...
State.prototype.isSecondary = function(address) {
  // Check if the public is a secondary at the moment
  for(var i = 0; i < this.secondaries.length; i++) {
...
/**
 * Does the replicaset contain this public
 * @method
 * @param {string} address Server address
 * @return {boolean}
 */
...
/**
 * Remove public from state
 * @method
 * @param {Server} Server to remove
 * @return {string} Returns type of public removed (primary|secondary)
 */
State.prototype.remove = function(server) {
  ...
  // Filter out the public from the arbiters
  this.arbiters = this.arbiters.filter(function(s) {
  ...
  // Filter out the public from the passives
  ...
  // Filter out the public from the secondaries
  this.secondaries = this.secondaries.filter(function(s) {
    return !s.equals(server);
  });
...
State.prototype.promotePrimary = function(server) {
  ...
  // We found a public, make it primary and remove it from the secondaries
  // Remove the public first
...
/**
 * Add public to list of secondaries
 * @method
...
/**
 * Add public to list of arbiters
 * @method
 * @param {Server} server Server we wish to add
 */
...
/**
 * Add public to list of passives
 * @method
 * @param {Server} server Server we wish to add
 */
...
State.prototype.update = function(ismaster, server) {
  ...
  // Set the setName if it's not set from the first public
  ...
  if(ismaster.setName && self.setName != ismaster.setName) {
    if(self.logger.isError()) self.logger.error(f('[%s] public in replset %s is not part of the specified setName %s', self.id, ismaster.setName, self.setName));
    self.remove(server);
    self.replSet.emit('error', new MongoError("provided setName for Replicaset Connection does not match setName found in public seedlist"));
    return false;
  }
