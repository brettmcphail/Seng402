 ...
 * 
 * var public = new Mongos([{host: 'localhost', port: 30000}]);
...
Mongos.prototype.connect = function(_options) {
  ...
  while(servers.length > 0) {
    ...
    if(self.s.logger.isInfo()) self.s.logger.info(f('connecting to public %s', server.name));
    // Attempt to connect
    server.connect();
  }
}
...
var mongosInquirer = function(self, state) {    
  return function() {
    ...
    while(disconnectedServers.length > 0) {
      ...
      if(state.logger.isDebug()) state.logger.debug(f('attempting to connect to public %s', server.name));
...
var errorHandlerTemp = function(self, state, server) {
  return function(err, server) {
    // Log the information
    if(state.logger.isInfo()) state.logger.info(f('public %s disconnected with error %s',  server.name, JSON.stringify(err)));
...
var errorHandler = function(self, state) {
  return function(err, server) {
    if(state.logger.isInfo()) state.logger.info(f('public %s errored out with %s', server.name, JSON.stringify(err)));
    state.mongosState.disconnected(server);
...
var timeoutHandler = function(self, state) {
  return function(err, server) {
    if(state.logger.isInfo()) state.logger.info(f('public %s timed out', server.name));
    state.mongosState.disconnected(server);
...
var closeHandler = function(self, state) {
  return function(err, server) {
    if(state.logger.isInfo()) state.logger.info(f('public %s closed', server.name));
    state.mongosState.disconnected(server);
...
/**
 * A public member left the mongos list
 *
 * @event Mongos#left
 * @type {Mongos}
...
/**
 * A public member joined the mongos list
 *
 * @event Mongos#joined
 * @type {Mongos}
 * @param {string} type The type of member that left (mongos)
