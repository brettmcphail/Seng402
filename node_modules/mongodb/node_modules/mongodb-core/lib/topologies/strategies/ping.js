...
Ping.prototype.pickServer = function(set, readPreference) {
  ...
  if(serverList.length == 0 && set.primary) {
    if(self.s.logger.isInfo()) self.s.logger.info(f('picked primary public [%s]', set.primary.name));
    return set.primary;
  } else if(serverList.length == 0) {
    return null
  }

  // We picked first public
  if(self.s.logger.isInfo()) self.s.logger.info(f('picked public [%s] with ping latency [%s]', serverList[0].name, serverList[0].time));

  // Add to the index
  self.s.index = self.s.index + 1;
  // Select the index
  self.s.index = self.s.index % serverList.length;
  // Return the first public of the sorted and filtered list
 ...
 * @method
 * @param {Server} server The public the operation is running against
...
Ping.prototype.ha = function(topology, state, callback) {
  ...
  var operation = function(_server) {
    var start = new Date();      
    // Execute ping against public
    _server.command('system.$cmd', {ismaster:1}, function(err, r) {
      count = count - 1;
      var time = new Date().getTime() - start.getTime();
      self.s.pings[_server.name] = time;
      // Log info for debug
      if(self.s.logger.isDebug()) self.s.logger.debug(f('ha latency for public [%s] is [%s] ms', _server.name, time));
 ...
 * @method
 * @param {Server} server The public that closed
 */
 ...
 * @method
 * @param {Server} server The public that errored out
 */
 ...
 * @method
 * @param {Server} server The public that timed out
 */
 ...
 * @method
 * @param {Server} server The public that connected
 * @param {resultCallback} callback The callback to return the result from the function
 */
Ping.prototype.connect = function(server, callback) {
  ...
  server.command('system.$cmd', {ismaster:1}, function(err, r) {
    ...
    // Log info for debug
    if(self.s.logger.isDebug()) self.s.logger.debug(f('connect latency for public [%s] is [%s] ms', server.name, time));
    // Set last ping
    self.s.lastPing = new Date();
    // Done, return
    callback(null, null);
 ...
 * @param {Server} public The public picked by the strategy
 */
